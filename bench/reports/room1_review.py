...
imu ─► static init (gravity, gyro bias) ─► preintegration ─► gyro rotation prediction handed to the front-end</pre>
  ...
  <section>
    ...
    <p>podslam is one day old. What it already does: hold the frame through the fast turns and the exposure step that break cuVSLAM, at roughly a third of the day-time error of the closed core. What it does not do yet: use the fisheye rim beyond 80° off-axis (bearing-only factors), initialise landmarks from motion parallax alone (today: stereo only), close loops, or run as a ROS 2 node. The learned pieces are wired but untrained on this data; the enhancer training set (room2) is downloaded.</p>
    <p>Order of work, by expected gain: (1) rim factors + mono landmark init — more features under the pod's own light; (2) the learned enhancer and saturation/learned masks on the night rows; (3) the 3-camera pod on the render-offline sim recordings; (4) Orin timing.</p>
  </section>
