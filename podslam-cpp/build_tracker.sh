...
set -euo pipefail
cd "$(dirname "$0")/.."
docker run --rm ${GUARD_DOCKER_ARGS:-} ${PODSLAM_CPU_QUOTA:+--cpus=$PODSLAM_CPU_QUOTA} -e OPENCV_CPU_DISABLE="${OPENCV_CPU_DISABLE:-}" -e PODSLAM_SKIP_BUILD="${PODSLAM_SKIP_BUILD:-}" -e PODSLAM_DW="${PODSLAM_DW:-}" -e PODSLAM_DENSE="${PODSLAM_DENSE:-}" -e PODSLAM_INIT_MODE="${PODSLAM_INIT_MODE:-}" -e PODSLAM_WORLD="${PODSLAM_WORLD:-}" -e PODSLAM_CV_THREADS="${PODSLAM_CV_THREADS:-}" -v "$PWD:/work" -v /tmp/claude-1001:/tmp/claude-1001 -w /work fisheye/gtsam-dev bash -c '
  set -e
